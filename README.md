...
P3 добавляет маршрут «иерархическая теория → замороженный набор экспериментов → связанное с ним измерение → пересмотр объяснений». Отчёты и прогнозы проверяются по дайджестам; исход вне поддержки всех моделей требует расширения представления. Синтетический benchmark проверяет рождение взаимодействующих координат, но не мировой научный приоритет. Идентификаторы экзопланетного портфеля согласованы, но для двух из пяти объяснений отсутствуют обоснованные прогнозы; до их появления выбор следующего эксперимента заблокирован. Подробности: [локальная проверка P3](reports/p3/RESULT_RU.md).

Дополнительный маршрут доступности наблюдений сохраняет паспорт каждой входной строки, не отбрасывая объект из-за отсутствия части величин. Верхние и нижние пределы учитываются как ограничения, а не центральные измерения. Иерархический компилятор разделяет общую структуру, состояние звезды/группы, локальные режимы и допустимые области; гипотеза получает условия как опровержения, так и проверки совместимости с данными. [Локальный результат P2](reports/exoplanet/P2_RESULT_RU.md) показывает полезность прогноза по другим планетам той же звезды, но не устанавливает причинность или новый закон.

В репозиторий включён предоставленный [снимок наблюдений NASA PSCompPars](data/observations/nasa_exoplanet/README_RU.md): 6366 строк и 84 столбца, с исходными байтами и SHA-256. [Локальная проверка](reports/exoplanet/RESULT_RU.md) воспроизвела известную размерностную структуру, но не подтвердила новой зависимости. Это тот же каталог, что использован автором патча, а не независимые новые наблюдения.

Atlas может начинать анализ остатка с постоянной базовой модели, без искусственной входной оси. Ядро исследует совместные группы неактивных координат: взаимодействие двух величин может оказаться полезным, даже если каждая по отдельности не проходит проверку. Выборочное углубление, повторные проверки на разделённых группах и сокращение лишних координат ограничивают необоснованное расширение модели.

Активация представления, фактическое присутствие оси в формуле и установленная причинность — разные статусы. Производные взаимодействия сохраняются локально для исследования, без автоматического продвижения в канонический реестр. Исчерпание вычислительного бюджета не означает исчерпания научного пространства.

Добавлены экспериментальные маршруты NASA-экзопланет и расширения языка `LATENT / EXPONENTIAL / KERNEL`. Приложенный к патчу отчёт не заявляет нового закона; исходный каталог наблюдений в архив не включён. Инструкции запуска и границы проверки приведены в [техническом руководстве](TECHNICAL_GUIDE_RU.md).

## Родословная гипотез и тип источника наблюдений

Atlas различает неудачу конкретной формулы и опровержение всего семейства гипотез: `FALSIFIED FORM != FALSIFIED FAMILY`. В существующее ядро `AdaptiveResearchKernelOwner` добавлены связанные дайджестами поколения: провал формы может оставить семейство открытым для другого представления, а опровержение инварианта закрывает семейство. Старая форма и одно лишь изменение её коэффициентов не считаются новой гипотезой; проверяются идентичность семейства и структурная идентичность формы. При недостатке сведений продолжение блокируется. Активное семейство не означает доказанный закон.

Общий контракт источника разделяет вычислительный источник ответов `FORWARD_ORACLE` и архив наблюдений `OBSERVATIONAL_ARCHIVE`. Архиву нельзя приписывать способность отвечать на произвольные вмешательства: заранее фиксируются доступные естественные выборки и преобразования, затем требуется связанное с протоколом измерение. Правила родословной и источников не выбирают алгоритм по названию науки; предметный адаптер отвечает за смысл и получение данных.

В репозиторий включены предоставленный SGS-план и три уже полученных фрагмента JHTDB isotropic8192 с квитанциями и проверенными хешами. Программа получения данных поддерживает возобновление по проверенным блокам. Производные целевые величины и результаты проверки новых гипотез в квитанции получения ещё отсутствуют. Само утверждение автора о порядке «freeze → получение» не становится независимо подтверждённым от проверки SHA-256. Подробности и ограничения: [локальная проверка V3](reports/turbulence/V3_INTEGRATION_RU.md).

## Общий исследовательский цикл для разных наук

Один и тот же механизм P3 выбирает различающий эксперимент, фиксирует прогнозы и пересматривает объяснения для разных предметных областей. Квалификация проверяет четыре синтетических портфеля: астрономию с категориальными исходами, материаловедение с гауссовыми прогнозами, гидродинамику с интервалами и математику с условными метками свидетельств/контрпримеров. Последний контроль не выполняет доказательство теорем. Замена только названия науки не должна менять выбор эксперимента или его оценку полезности.

Для существующих исследований добавлена общая диагностика готовности: есть ли теория, конкурирующие объяснения, модели экспериментов и полный набор прогнозов. NASA-портфелю пока недостаёт двух прогнозов; маршруту JHTDB и фармацевтическим материалам нужен явный контракт P3. Наличие программы загрузки DNS не означает, что измерение уже выполнено. После успешной фиксации требуется новое измерение, связанное именно с этим экспериментом: старые наблюдения не становятся новым доказательством задним числом.

Предметные адаптеры задают величины, единицы, схемы данных и интерфейс измерения; выбор эксперимента и пересмотр объяснений выполняют общие владельцы. Диагностика находится в модуле квалификации, а не в новом автономном научном движке. Переносимость проверенных контрактов не означает решение всех наук или открытие закона. Запуск и результаты: [техническое руководство](TECHNICAL_GUIDE_RU.md#междисциплинарная-квалификация-и-готовность-портфелей), [локальный отчёт](reports/p3/UNIVERSAL_PORTFOLIO_RESULT_RU.md).
...
→ translation/algebra meta-primitives
→ Atlas-generated operator language
→ identifiability screen
→ sparse adaptive multi-axis support search
→ sealed OOD verification
```

Контрольный Level 3 рождает 20 типизированных signatures на канал, допускает продолжение rank-shell search без глобального потолка и переключает большие пространства кандидатов на sparse forward/backward search. Ресурсный trial budget ограничивает конкретное выполнение, но не объявляет научный предел размерности или порядка операторов.
...
Успешный контроль означает воспроизводимое рождение и выполнение языка внутри controlled reference world. Он не означает новый физический закон, мировую математическую новизну, изобретение дифференциального исчисления «из ничего» или причинное доказательство выбранных осей: `REPRESENTATION_ACTIVATED != CAUSALLY_ESTABLISHED`.
...
- [Адаптивная оценка данных, песочница и совет гипотез](#адаптивная-оценка-данных-песочница-и-совет-гипотез)
- [Рождение языка функций по структуре остатка](#рождение-языка-функций-по-структуре-остатка)
...
Atlas не обязан достраивать модель строго по одной координате за цикл. Если
структура задачи проявляется только в совместном изменении нескольких величин,
междисциплинарном мосте или взаимодействии высокого порядка, система может
номинировать сразу конечный набор взаимосвязанных исследовательских осей:
...
Кардинальность рождения определяет сама типизированная структура задачи, а не
предустановленное число. Поиск остаётся разреженным: Atlas открывает локальные
многомерные подпространства, а не подставляет все 655 осей одновременно.
Открытость означает возможность продолжать расширение без научного фиксированного
потолка; каждый конкретный запуск и каждый пакет рождения конечны и ограничены
явным вычислительным бюджетом. Каждая новая ось проходит собственную типизацию,
проверку происхождения и жизненный цикл admission/promotion, поэтому пакетная
номинация не обходит доказательные барьеры и не является автоматическим
признанием нового закона.

## Текущий сертифицированный снимок

Запечатанное состояние 0.15.29.0 содержит:
...
## Адаптивная оценка данных, песочница и совет гипотез

Atlas 0.15.28.0 добавляет неавторитетный исследовательский слой `RESEARCH-TRIAGE-SANDBOX/1.0.0` рядом с неизменным конвейером U0–U10. Единственным владельцем научного продвижения остаётся `SCIENTIFIC-PROMOTION-CORE/9.2.0`: песочница может сохранить слабую или необычную гипотезу, определить приоритет дальнейшей работы, предложить измерения и зарегистрировать решение эксперта, но не может присвоить `U6=PASS`, `LAW_CANDIDATE` или обойти любой обязательный барьер.

### Динамические уровни качества данных

Качество набора данных оценивается по числу наблюдений и относительной неопределённости. Диагностическая информационная плотность

```text
I_data = min(1, log(1+n)/log(1001)) / (1 + 10 * relative_uncertainty)
```

используется только для маршрутизации и ранжирования. Она не заменяет статистическую проверку, перестановочную калибровку или доказательные барьеры.

| `quality_tag` | Условие | Порог песочницы | Маршрут |
|---|---|---|---|
| `STRICT` | `n > 1000`, неопределённость `< 1%` | `rho*=0.05`, `sigma*=0.05` | допускается к обычному строгому конвейеру при прохождении всех U-барьеров |
| `EMPIRICAL` | промежуточное качество | `rho*=0.15`, `sigma*=0.10` | `PHENOMENOLOGICAL_MODEL`, паспорт и метаанализ |
| `EXPLORATORY` | `n < 30` или неопределённость `> 10%` | `rho*<=0.30`, `sigma` не обязателен | `STRUCTURAL_ANOMALY`, только песочница, потолок до U4 |

Если неопределённость не объявлена, Atlas закрывается в безопасную сторону со статусом `UNCERTAINTY_UNDECLARED_ASSIGNED_EXPLORATORY`, а не считает данные точными.

### Карта исследовательской песочницы и DPI

Для сортировки кандидатов рассчитывается Discovery Probability Index:

```text
DPI = normalized(
    W_fit * (1-rho_adj)
  + W_simplicity * Simplicity
  + W_cross * CrossConsistency
  - W_fwer * FWERPenalty
)
```

Стандартные нормированные веса — `0.35 / 0.20 / 0.30 / 0.15`, а `FWERPenalty = clip(p_FWER / alpha_FWER, 0, 1)`. Значение `DPI >= 0.70` даёт рабочую метку `PROMISING_REJECT`: гипотезу разумно исследовать дальше, но она не проходит U5 или U6. Браузерный проектор в `static/index.html` отображает настоящий JSON `rank_exploration_sandbox` по координатам `accuracy`, `simplicity`, `uniqueness`; положение точки на 3D-карте не является научным доказательством.

Планировщик «что если?» анализирует наблюдавшиеся и целевые диапазоны осей, чувствительность и неопределённость, после чего предлагает измерения для незакрытых областей. Такая заявка всегда содержит `world_result_observed=false` и `proposal_is_evidence=false`, пока результат не получен в реальном эксперименте.

### Совет гипотез и обучение по решениям эксперта

Поддерживаются действия `NOTE`, `SPONSOR_REVIEW`, `REJECT`, `FRESH_IDEA_LINK` и `REQUEST_SANDBOX_DIMENSION_EXCEPTION`. Они записываются в контентно-адресуемый журнал внешнего изменяемого состояния. `SPONSOR_REVIEW` возвращает гипотезу к сбору доказательств, но не проходит U6; исключение размерности разрешает исследование только в песочнице, но не закрывает U2.

После как минимум восьми решений `SPONSOR_REVIEW`/`REJECT` с полными снимками признаков Atlas может подготовить рекомендацию новых весов DPI. Она применяется только отдельным явным действием и меняет исключительно ранжирование песочницы, а не правила научной истинности.

Новые read-only методы API:

```text
get_research_triage_contract
assess_research_data_quality
rank_exploration_sandbox
build_hypothesis_passport
propose_sandbox_what_if_experiments
get_hypothesis_council_state
recommend_hypothesis_council_weights
```

Методы изменения внешнего состояния: `record_hypothesis_council_action` и `apply_hypothesis_council_weight_recommendation`. Проверка подсистемы запускается командой `make research-triage`.

Главный инвариант выпуска:

```text
строгая экспертиза U0–U10 != ранжирование песочницы != мнение эксперта
```

## Рождение языка функций по структуре остатка

Atlas 0.15.27.0 расширяет существующий маршрут `QUERY-DRIVEN-RESEARCH/1.2.0` и ядро `PHI-MATHEMATICAL-INVENTION-KERNEL/1.1.0`, не создавая параллельного оптимизатора или второго владельца научного поиска. Сначала точное рациональное размерностное ядро фиксирует координаты `Π₁, …, Πₚ`, затем Query проверяет базовую полиномиальную грамматику. Новый исполняемый язык разрешается породить только тогда, когда строго out-of-fold остаток остаётся существенным и содержит операционно-специфическую структуру выше multiplicity-aware порога.

Текущий конечный язык запроса способен порождать конкурирующие представления семейств `RATIONAL`, `EXPONENTIAL`, `LOGARITHMIC`, `PERIODIC`, `PIECEWISE`, `KERNEL` и `LATENT`. Это названия вычислимых представлений, а не известных законов и не исчерпывающий каталог математики. Стандартизация, пороги, центры RBF и латентные проекции обучаются только на тренировочной части каждого fold. Рациональная модель закрывается как неопределённая, если её знаменатель попадает в защищённую окрестность нуля.

Рождение языка включено внутрь адаптивной нулевой проверки. Для каждой перестановки цели полностью повторяется цепочка:

```text
полиномиальная поверхность
→ out-of-fold остаток
→ проверка сигнала операции
→ необязательное рождение языка
→ повторная аппроксимация и ранжирование всей порождённой поверхности
```

| Контроль | Полиномиальный OOF NRMSE | OOF NRMSE порождённого языка | Результат |
|---|---:|---:|---|
| скрытая периодическая поверхность | `0.9664343` | `0.1318550` | язык `PERIODIC`, снижение ошибки на 86,36% |
| локальная двумерная поверхность | `0.8005488` | `0.3230948` | язык `KERNEL`, снижение ошибки на 59,64% |
| полиномиальный multi-Π контроль | `0.0177841` | без изменения | ненужный язык не порождается |
| бесструктурный шум | около `1` | неприменимо | максимальный сигнал `0.1487225` ниже порога `0.2067587` |

Плохая полиномиальная аппроксимация сама по себе не является основанием расширять язык. Эти результаты квалифицируют механизм представлений, но не доказывают новый закон, математическую новизну, превосходство Atlas как универсального оптимизатора или воспроизводимость на реальных данных. Научная перепись по-прежнему имеет `U5 = 0`.
...
Этот раздел описывает метод и предполагаемую интеграцию с Atlas. Производственный владелец версии 0.15.29.0 реализует прямое точное рождение π-групп Бекингема из уже зарегистрированных размерных величин. Он **пока не предоставляет полный обратный процесс регрессии отсутствующей константы как отдельного владельца API**. Статья задаёт проверенную спецификацию метода и набор контролей; поставляемая перепись скалярных законов реализует более узкий прямой процесс.
...
Зависимости среды выполнения перечислены в [pyproject.toml](pyproject.toml): NumPy, SciPy, SymPy, PyYAML, cryptography и pypdf. Численное ядро выпуска квалифицировано и зафиксировано на NumPy `2.4.2`, SciPy `1.17.0` и SymPy `1.14.0`: это предотвращает изменение контентных дайджестов фронтира при автоматической установке более новых версий на CI.
...
Версия компонента (`0.10.0`), версия AI-приёмки (`15.10.6`), схема компонента (`6.0.0`), схема состояния (`5`) и системный выпуск Atlas (`0.15.29.0`) намеренно являются разными осями идентичности.
...
Актуальный состав тестов показывает `make collect`; итоговая квалификация и аудиты выпуска должны завершаться без ошибок.
...
### Фронтир проходит локально, но не проходит в GitHub Actions

Проверьте фактически установленные версии NumPy, SciPy и SymPy. Контентно-адресуемый численный снимок должен воспроизводиться на квалифицированном стеке из `pyproject.toml`; установка более свежей версии без пересборки seal может изменить младшие разряды вычислений и, следовательно, SHA-256 квитанций.

При read-only replay prior-art всегда проверяется относительно SHA-256 реально запечатанного `ATLAS_ACTIVE_CANDIDATES_CURRENT.jsonl`. Отдельно Atlas требует полного совпадения типизированных идентичностей кандидатов, классов, областей, владельцев и эпистемических статусов с пересчитанным фронтиром. Поле `replay_sha256` остаётся диагностикой: его побитовое несовпадение между BLAS/libm-платформами не подменяет и не изменяет зафиксированный ledger, но любое семантическое расхождение закрывает квалификацию с отказом.
...
```text
Atlas / Φ-Compiler / ScienceAtlas, системный выпуск 0.15.29.0,
...
## Реальный blind DNS-эксперимент по замыканию турбулентности

Atlas может исследовать переносимые модели неразрешённой турбулентной динамики по реальным периодическим DNS-снимкам. Адаптер вычисляет аттестованный coarse-graining residual из исходных трёхмерных полей скорости, после чего ядро рождает и проверяет локальные operator-координаты без каталога известных RANS/LES closure-моделей и без переданных извне производных-признаков.

Общий режим `DIRECT_FIELD_VALUE` использует target только как наблюдаемый response и жёстко исключает его из predictor language. Discovery и sealed holdout имеют разные `regime_id`; sealed regime не участвует в рождении осей, а детерминированный null-control включён по умолчанию.

Новый bootstrap автоматически получает шесть frozen raw-velocity cutout из Johns Hopkins Turbulence Database: четыре discovery snapshot из `isotropic1024coarse` (`R_λ ≈ 433`) и два sealed snapshot из `isotropic4096` (`R_λ = 610.57`). Каждый запрос остаётся ниже лимита публичного testing token, выполняется последовательно, а полученные `.npz` и provenance receipts связываются SHA-256. Синтетическая подмена реальных данных запрещена.
...
Для собственного JHTDB token задайте `JHTDB_TOKEN`. Frozen plan можно проверить без сети:

```bash
python -m evaluation.fetch_jhtdb_real_snapshots --dry-run
```

Подробная инструкция находится в [RUN_TURBULENCE_DNS_CLOSURE_RU.md](RUN_TURBULENCE_DNS_CLOSURE_RU.md). `PASS_PROTOCOL_INTEGRITY` подтверждает только целостность blind/freeze/provenance-протокола. Даже переносимый кандидат остаётся `NOT_LAW` и требует независимой фальсификации.

## Цикл исследования с выполнением измерений

`LawSpaceAPI.run_closed_loop_research(request, measurement_adapter=adapter)` соединяет анализ остатка, рождение вычисляемой оси, выбор различающего измерения, получение ответа адаптера и повторное обучение. Финальная контрольная выборка фиксируется хешем до поиска и раскрывается только после замораживания итоговой модели. Новый маршрут доступен через Python с явно подключённым адаптером; обычный текстовый `research` пока не подключает его автоматически.

Проверка полного цикла в отдельном процессе:

```bash
python -m evaluation.closed_loop_research_experiment --output /tmp/atlas-closed-loop-control.json
```

Текущий класс новых осей — вычисляемые координаты локализации остатка на основе существующего `AxisModelingOwner`. Цикл не изобретает произвольные физические наблюдаемые. Синтетические проверки подтверждают выполнение переходов без передачи скрытых параметров исследовательскому владельцу; они не подтверждают открытие закона природы. Продвижение закона и причинная интерпретация остаются закрытыми.
