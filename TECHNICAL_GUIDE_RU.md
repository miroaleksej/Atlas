# Atlas 0.15.27.0: техническое руководство
...
Фактически загружаемый реестр выпуска 0.15.27.0:
...
Ожидаемая версия — `0.15.27.0`. Актуальное число тестов следует проверять командой `pytest --collect-only -q`: оно растёт вместе с квалифицированными владельцами и сценариями.
...
В выпуске 0.15.27.0 собирается **114 тестов**. Ниже описан каждый тест: что проверяется, каким способом и какой результат считается успешным. Идентификатор после имени файла можно передать `pytest` для отдельного запуска:
...
- `test_example_notebook_is_current_release` — исполняет все code cells notebook и проверяет metadata выпуска 15.25.0, а также фильтрацию локальных артефактов. Ожидается корректный notebook формата 4, совместимый с 0.15.27.0.
