...
## P3: теория, заморозка эксперимента и пересмотр

`api.freeze_phi_observational_theory_round(theory=theory, cost_budget=budget)` принимает результат иерархического компилятора с `experimental_models`. Для каждого эксперимента нужны уникальный ID, наблюдаемая величина, положительная конечная стоимость и прогноз для каждого конкурирующего объяснения. Поддерживаются категориальные, интервальные и гауссовы прогнозы. Отбор использует эвристическую различимость прогнозов с учётом стоимости, а не эмпирическое доказательство.

`api.revise_phi_observational_theory(frozen_experiment=frozen, measurement=measurement)` требует совпадения `experiment_id` и `freeze_digest`. Замороженный отчёт и его вложенный payload проверяются по дайджестам, выбранный эксперимент должен принадлежать зафиксированному набору. Измерение содержит `value`; новый исход вне поддержки всех моделей приводит к `REPRESENTATION_EXPANSION_REQUIRED`, без выбора ближайшего объяснения. Веса сравнения не являются калиброванными байесовскими вероятностями или подтверждением закона.

`api.run_phi_representation_novelty_benchmark()` запускает синтетические контроли совместных взаимодействий, переноса без переобучения на контрольных данных, отсутствия рождения на шуме и полного цикла пересмотра. Это диагностический benchmark, не поиск неизвестного закона природы.

В `tests/test_observational_round_integrity.py` проверяются: неизвестный исход → расширение; изменение замороженного прогноза → отказ; отсутствие прогноза для объяснения → блокировка; отрицательная стоимость и неверный дайджест теории → отказ. Сохранены также тесты запрета внешнего подтверждения одним флагом и проверки полного внешнего receipt.

В текущем NASA-портфеле P3 идентификаторы согласованы, но для `H-KNOWN-STELLAR-STATE` и `H-HOST-EFFECTIVE-LUMINOSITY` пока нет обоснованных прогнозов: пять объяснений теории не покрыты тремя предложенными прогнозами. Локальный запуск возвращает `BLOCKED_INCOMPLETE_EXPERIMENT_PREDICTIONS`; результаты будущего эксперимента не подставлялись. Общий цикл проверяется отдельно на согласованных синтетических моделях. Данные и прежние P2-результаты сохранены.

## Междисциплинарная квалификация и готовность портфелей

Модуль `evaluation.long_horizon_blind_cycle_qualification` проверяет общий цикл P3 без отдельного алгоритма для каждой науки. Полный запуск:

```bash
PYTHONDONTWRITEBYTECODE=1 OPENBLAS_NUM_THREADS=1 \
python -m evaluation.long_horizon_blind_cycle_qualification \
  > /tmp/atlas-long-horizon.json

PYTHONDONTWRITEBYTECODE=1 PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 \
python -m pytest -q -p no:cacheprovider tests/test_universal_scientific_portfolio.py
```

Основной отчёт содержит 30 проверок; вложенные блоки `universal_scientific_portfolio_qualification` и `real_scientific_portfolio_ingress_qualification` — 22 и 12 соответственно. Эти числа обозначают проверки внутри отчёта, а не количество pytest-тестов всего проекта.

Синтетический блок использует четыре контракта: астрономический категориальный, материаловедческий гауссов, гидродинамический интервальный и математический с условными категориальными исходами. Все проходят через одни и те же владельцы теории, выбора эксперимента и пересмотра. Математические исходы здесь заданы контролем: генерация настоящего доказательства не проверяется. Дополнительный контроль меняет только метки наук и сравнивает выбранный эксперимент и полезность.

Гауссово распределение имеет полную математическую поддержку, но это не делает любой результат совместимым с моделью. Если все эвристические оценки не выше замороженного `compatibility_floor=0.01`, требуется `REPRESENTATION_EXPANSION_REQUIRED`, без назначения победителя. Это не логическая невозможность события и не калиброванный p-value.

Диагностический маршрутизатор `_route_real_scientific_portfolio` принимает идентификатор портфеля, область, происхождение и необязательную скомпилированную теорию. Он расположен в модуле квалификации и не является новым публичным API. Он не создаёт отсутствующие прогнозы и не выполняет внешние измерения.

| Состояние входа | Следующий обязательный этап |
|---|---|
| Нет теории P3 | `PORTFOLIO_P3_THEORY_CONTRACT_REQUIRED` |
| Меньше двух объяснений | `PORTFOLIO_COMPETING_EXPLANATIONS_REQUIRED` |
| Нет моделей экспериментов | `PORTFOLIO_EXPERIMENTAL_MODELS_REQUIRED` |
| Прогнозы не покрывают объяснения | `PORTFOLIO_PREDICTION_CONTRACT_REQUIRED` |
| Эксперимент успешно зафиксирован | `PORTFOLIO_FROZEN_FRESH_MEASUREMENT_REQUIRED` |
| Иная блокировка выбора, включая бюджет | `PORTFOLIO_SELECTION_BLOCKED` |

NASA-портфель проверяется по сохранённой реальной теории; в текущих данных отсутствуют прогнозы для двух объяснений. JHTDB представлен определением маршрута и инструкцией, а не автоматически новым измерением. Фармацевтические артефакты при наличии проходят ту же диагностику. Входные файлы сверяются с фактическими SHA-256, а не только со строками `HASHES.txt`. Дайджест подтверждает целостность, но не научную истинность. Ранее полученное evidence не переиспользуется для нового post-freeze измерения.

Тесты в `tests/test_universal_scientific_portfolio.py`:

- `test_long_horizon_universal_scientific_portfolio_qualification`: выполняет четыре контролируемых портфеля, сравнивает владельцев, исходы пересмотра и инвариантность к названию науки; ожидает 22/22.
- `test_real_scientific_portfolios_enter_shared_p3_readiness_contract`: читает реальные артефакты репозитория, проверяет общий маршрут и сохранение границы будущего измерения; ожидает успешную диагностику, не научное продвижение.
- `test_portfolio_readiness_preserves_missing_evidence_gate`: пять параметризованных случаев от отсутствующей теории до успешного freeze; каждый должен остановиться на соответствующем недостающем этапе.
- `test_portfolio_seal_requires_matching_file_bytes`: отсутствие хеш-реестра и изменение байтов не дают подтверждённого дайджеста; точное совпадение даёт.
- `test_portfolio_missing_inputs_cannot_pass_qualification`: пустой каталог должен вернуть блокировку, а не успешную квалификацию.
...
| Объявленная область неудачи | Состояние семейства | Состояние формы |
|---|---|---|
| `FORM_ONLY`, `FORM_OR_REPRESENTATION`, `REPRESENTATION`, `TRANSFER` | `ACTIVE_REPRESENTATION_EXPANSION` | `RETIRED_FAILED_FORM` |
| `FAMILY_INVARIANT` | `FALSIFIED_FAMILY` | `FALSIFIED_FORM` |
| Не определена | `CANDIDATE_PENDING_FAILURE_SCOPE_ADJUDICATION` | `UNRESOLVED_FORM` |
| Отличающаяся дочерняя форма после синтеза | `ACTIVE_TESTABLE_FAMILY` | `TESTABLE_NOT_LAW` |

Область неудачи задаётся в контексте доказательств, а не угадывается по подстроке статуса. Дочерней форме нужны собственные ID, дайджест и структурный дайджест. Повторный полный дайджест или та же структура при новых коэффициентах отклоняются. Для типизированной мономиальной модели структура вычисляется по выражению, базису и размерностям коэффициентов без численных параметров и ошибок аппроксимации. Это не универсальный решатель алгебраической эквивалентности: неподдержанный язык требует отдельного структурного контракта. У старого родителя без структурной идентичности продолжение не разрешается автоматически.

Исполняемые точки интеграции — `ATTESTED_HYPOTHESIS_FORM_FAILURE`/`ATTESTED_REPRESENTATION_GAP` и scale-invariant продолжение primitive-field маршрута. Наличие поля lineage не означает автоматическую интеграцию со всеми API Atlas. Запись остаётся исследовательской, не открывает U5–U10 и требует свежей post-freeze проверки. Дайджесты связывают переданные записи, но сами не доказывают истинность внешнего измерения и не заменяют доверенное постоянное хранилище истории.

`tests/test_hypothesis_family_lineage.py` проверяет: раздельное закрытие формы и семейства; ссылку на родителя; неизменность логики для шести меток наук; запрет возвращения закрытого семейства; подмену или удаление дайджеста; изменение ID, формулировки и инвариантов; повтор формы и коэффициентный refit; приоритет опровержения семейства; отсутствие области отказа, доказательной ссылки или структурной идентичности; стабильность структурного хеша; реальный вызов API и несовпадение хеша входного evidence.

## Архив наблюдений и получение JHTDB

Контракт `source_capability` разделяет `FORWARD_ORACLE` и `OBSERVATIONAL_ARCHIVE`. Для совместимости отсутствие класса помечается как legacy-допущение о вычислительном источнике; новый интеграционный код должен объявлять класс явно. Архив не считается симулятором произвольных состояний. Замораживаются ID естественных выборок, локаторы, преобразования, описание наблюдаемой величины и владелец адаптера. Подмена ответа в описании выборки, повтор ID и несогласованный источник отклоняются.

Без сетевого доступа можно проверить готовый план:
...
`evaluation.freeze_observational_archive_protocol` — CLI над общим ядром: `--request FILE --output FILE`. Он не должен применяться для объявления уже просмотренных данных новыми. `evaluation.freeze_jhtdb_sgs_child_forms` извлекает формы из прежнего DNS-отчёта и связывает их с заранее заданным протоколом; исходный большой отчёт является локальным файлом и не поставляется в GitHub. Готовые квитанции сохраняются, а не пересоздаются задним числом.

Загрузчик `evaluation.run_fresh_jhtdb_sgs_observational_experiment` получает сырые поля, но не вычисляет целевые residual, не выбирает термы и не проверяет научные гипотезы. Он использует внешний `givernylocal` только при реальном получении; dry-run и офлайн-тесты его не требуют. `JHTDB_TOKEN` читается из окружения, не записывается в отчёты. По умолчанию повторная запись готовой acquisition-квитанции запрещена; проверенные блоки позволяют возобновлять незавершённое получение. Для новых запусков выбирайте внешние `--output-dir` и `--receipt`, чтобы не изменять запечатанные данные. Не повторяйте получение имеющейся выборки под видом независимой репликации.

В поставляемой квитанции уже есть три выборки и 24 блока получения. Их байтовая целостность проверяется офлайн. Это не аттестация происхождения внешнего сервиса и не доказательство того, что никто не просматривал данные до freeze. Полученный сырой массив не равен готовому исходу различающего эксперимента: нужен зафиксированный адаптер, правила классификации результата и проверка без настройки на этих данных.

Тесты загрузчика проверяют 24 запроса и размер блоков, перевод индекса snapshot на границе API, распознавание временных ошибок, возобновление checkpoint и отказ при пустом наборе осей. `test_jhtdb_freeze_integrity.py` дополнительно проверяет подмену содержимого при прежнем хеше, выход пути за каталог, точный бюджет повторов, все хеши сырых данных и воспроизведение выбора SGS через общий P3-владелец. Все эти проверки офлайн; они не выполняют новый мировой эксперимент.

## Неполные наблюдения и иерархическая теория

Маршрут `evaluation.exoplanet_availability_adaptive` сохраняет все строки каталога в паспортах доступности. Вычисления выполняются в совместимых подпространствах имеющихся величин. Верхние/нижние пределы остаются неравенствами, модельные оценки не переименовываются в измерения. Это дополнительный режим, а не подмена complete-case протокола первого эксперимента.

```bash
python -m pip install -e '.[exoplanet]'
python -m evaluation.exoplanet_availability_adaptive \
  data/observations/nasa_exoplanet/PSCompPars_2026.09.20_07.33.39.csv \
  --out /tmp/atlas-availability.json \
  --passports-out /tmp/atlas-passports.csv
```

`PredictionFalsificationOwner` формирует контракт опровержения и контракт проверки совместимости кандидата с неопределённостями, ограничениями и областью применимости. Изменять модель по раскрытым контрольным данным нельзя. Возможное сужение области гипотезы не означает, что она прошла исходную проверку.

`TheoryCompilerKernel.hierarchical_observational_theory` компилирует четыре слоя: общую структуру, состояние группы/звезды, локальную структуру режима и допустимые области наблюдений. Компиляция проверяет объявленные контракты, но сама по себе не подтверждает истинность данных. `fresh_external_confirmation=True` без верифицированного доказательства отклоняется; одного флага недостаточно для внешнего подтверждения.

P2 добавляет тесты паспортов, неполных и ограниченных измерений, разделения ATTACK/DEFENSE, прогнозов удержанной планеты по остальным планетам её звезды и контрактов иерархической теории. Дополнительный `test_external_confirmation_cannot_be_granted_by_caller_flag` проверяет отказ при попытке получить подтверждённый статус одним флагом. Полный локальный результат описан в `reports/exoplanet/P2_RESULT_RU.md`.

## Углублённое исследование остатка и совместное рождение осей

`advance_adaptive_research` допускает пустой `predictor_variables`: начальная модель тогда содержит только свободный член. `dormant_axis_variables` задаёт доступные неактивные координаты. Ядро проверяет совместные зоны, а не только последовательное добавление одиночных осей, затем углубляет перспективные зоны и удаляет избыточные координаты. `axis_birth_trial_budget` ограничивает текущую порцию вычислений, а не число возможных научных направлений.

Проверки устойчивости используют повторные групповые разделения discovery-данных. Точный итоговый набор проходит повторную проверку после сокращения. Активированные оси и эффективный состав формулы отражаются раздельно; взаимодействия, степени и обратные величины могут сохраняться как `RESEARCH_LOCAL_DERIVED_AXIS_CANDIDATE`. Это не причинное доказательство и не каноническое продвижение.

Внешнее подтверждение стабильности от `AxisModelingOwner` требует полного `source_receipt`, совпадения канонического дайджеста, владельца, улучшения, привязки оси через `source_axis_mapping` и непересекающихся обучающих/проверочных режимов. Одна строка `source_digest` не принимается. Проверка целостности отчёта не удостоверяет истинность измерений и не заменяет независимую аттестацию мира.
...
В `tests/test_adaptive_axis_discovery.py` проверяются старт без активного предиктора и совместное рождение взаимодействующих осей: ожидаются выбранная пара, эффективный состав формулы и успешная проверка её устойчивости. Тест `test_external_stability_requires_matching_complete_owner_receipt` проверяет отказ для одного дайджеста, изменённого источника, неверной привязки оси и использования sealed-данных; согласованный контрольный отчёт проходит проверку целостности.

Числа NASA в `reports/NASA_EXOPLANET_2026_BLIND_CURRENT.md` относятся к исследованию автора патча. При установке они не становятся результатом нового локального запуска. Новый физический закон в этом отчёте не установлен.

## Исследовательский цикл с измерительным адаптером

Вызов `api.run_closed_loop_research(request, measurement_adapter=adapter)` выполняет конечный цикл на основе `AxisModelingOwner`: обучает базовую модель, анализирует остаток, предлагает вычисляемую координату, выбирает ещё не измеренную точку по расхождению предсказаний, получает измерение, проверяет его привязку к запросу, добавляет его в обучающую часть и повторяет обучение. Каждый шаг сохраняется с хешем исходной модели, запроса, ответа и пересмотренной модели.

В `request` передаются:

- `dataset`: набор `AxisModelingDataset` с `dataset_id`, `observable_id`, `observable_units`, `y`, `regime_ids`, `axes`, `provenance` и необязательной `sigma`;
- `validation_regime_ids`: явно выделенные режимы внутренней проверки; они участвуют в выборе кандидатов и не являются финальной независимой проверкой;
- `holdout_dataset_digest`: канонический `digest_payload` финального набора, зафиксированный до поиска;
- `measurement_budget`: положительное целое число измерений в этом запуске;
- `final_nrmse_tolerance`: положительный порог итоговой ошибки, по умолчанию 0.25;
- `seed`: воспроизводимость поиска.

Адаптер реализует `measure(request)` и `reveal_holdout(request)`. Ответ измерения содержит `request_digest`, точные `axis_values`, `observed`, `provenance`, `digest` и `sigma`, если исходный набор содержит измерительные неопределённости. Ответ финальной проверки содержит `request_digest`, `dataset`, `provenance`, `digest`. Поле `digest` вычисляется через `digest_payload` без самого этого поля. Несовпадение запроса, координат, хеша контрольного набора, единиц или нечисловой ответ останавливают цикл. Хеш подтверждает целостность, но не является аттестацией реального прибора.

Финальный набор раскрывается однократно после выбора модели. Повторно обучаться на нём в этом запуске нельзя. Его режимы и точки должны отличаться от использованных при разработке модели. Приборный адаптер подключается явно; CLI `research` автоматически его не создаёт.

```bash
PYTHONDONTWRITEBYTECODE=1 OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 \
python -m evaluation.closed_loop_research_experiment \
  --output /tmp/atlas-closed-loop-control.json

PYTHONDONTWRITEBYTECODE=1 PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 \
python -m pytest -q -p no:cacheprovider tests/test_closed_loop_research.py
```

Демонстрационная среда вычисляет ответы в отдельном процессе. Формула и случайно выбранные параметры среды не передаются исследовательскому владельцу. Это разделение интерфейсов, не защита от злонамеренного чтения исходников. Класс среды специально соответствует доступной семье представлений: полиномиальный фон и локализованный отклик. Успех означает освоение этой синтетической задачи, а не научную новизну.

Новая ось имеет вид `exp(-0.5*((x-center)/width)^2)`; центр и ширина выбираются из данных. Ось безразмерна, вычисляется из измеряемого исходного признака, получает предварительный семантический допуск и сохраняет исследовательский статус. Изобретение совершенно нового физического способа измерения этим маршрутом пока не реализовано. Успех `CLOSED_LOOP_HELDOUT_SUPPORTED_NOT_LAW` требует хотя бы одного выполненного измерения, итоговой ошибки ниже порога и улучшения относительно базовой модели. Иначе возвращается `CLOSED_LOOP_REPRESENTATION_GAP`. Полная адаптивная калибровка по перестановкам не выполнена; продвижение закона запрещено.

Проверки `tests/test_closed_loop_research.py`:

- `test_loop_measures_revises_and_freezes_before_final_reveal`: три разных скрытых набора параметров; проверяет выполнение двух измерений, изменение модели, связи хешей и успех финального прогноза без продвижения закона.
- `test_unpredictable_or_shifted_world_is_not_supported`: шум и изменение зависимости на финальном наборе; ожидается сохранение нерешённого разрыва.
- `test_replayed_misbound_or_nonfinite_measurement_is_rejected`: старый запрос, неверная точка и NaN; ожидается отказ до раскрытия финальной выборки.
- `test_holdout_substitution_after_search_is_rejected`: подмена финальных наблюдений после поиска; ожидается ошибка предшествующего обязательства по хешу.
- `test_world_disallows_measurements_after_final_reveal`: демонстрационный адаптер закрывает возможность новых измерений после финального раскрытия.
