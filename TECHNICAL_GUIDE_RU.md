...
```text
научный вопрос + наблюдаемые + размерности + данные
                         │
                         ▼
              типизация и проверка входа
                         │
                         ▼
       локальные и междисциплинарные подпространства
                         │
          ┌──────────────┼────────────────┐
          ▼              ▼                ▼
 точные π-группы   поиск представления   владельцы областей
          └──────────────┼────────────────┘
                         ▼
          зафиксированный набор кандидатов
                         ▼
           сравнение и дизайн эксперимента
                         ▼
                 доказательства U0–U10
```
...
Фактически загружаемый реестр выпуска 15.24.0:
...
Ожидаемая версия — `15.24.0`. Полный набор содержит 100 тестов.
