...
Рождение осей имеет адаптивную кардинальность. Для конечного множества новых
осей (B_k), выбранного структурой задачи на цикле (k), разрешён переход

```math
\mathcal A_{k+1}=\mathcal A_k\cup B_k,\qquad 0\le |B_k|<\infty.
```

Ни `|B_k|=1`, ни другое фиксированное число осей на цикл не является
архитектурным требованием. Нормативная политика:
...
Следовательно, Atlas MAY номинировать за один исследовательский цикл несколько
взаимозависимых осей и higher-order interaction axes, если одноосевое
последовательное расширение разрушает или скрывает структуру задачи. Система
MUST NOT требовать жадного перехода «ровно одна новая координата за цикл».
Одновременно конкретный вычислительный цикл MUST материализовать только конечное
множество, а каждая новая ось MUST отдельно пройти типизацию, admission,
provenance и Dynamic Axis Lifecycle. Пакетное рождение MUST NOT обходить эти
барьеры и не означает пакетного научного продвижения.

Также разрешены переходы жизненного цикла:
