...
Родословная гипотез дополняет эти контроли разделением формы и семейства: неудачный перенос одной формы может требовать другого представления, но опровержение общего инварианта закрывает семейство. Поколения связываются дайджестами, неизменными формулировкой и инвариантами; для ребёнка проверяется отличие структуры, а не только численных коэффициентов. При отсутствии структурной идентичности продолжение блокируется. Общий механизм не устанавливает научную истинность и не заменяет оценку доказательств владельцем предметной области.

Контракт источника отдельно различает вычислительный источник произвольных ответов и архив естественных наблюдений. Для архива фиксируются выборки, преобразования и адаптер измерений, а не искусственные вмешательства, которых архив не способен выполнить. Получение сырого поля, выполнение измерительного адаптера, совместимость модели и научное продвижение остаются разными этапами. Подробная инструкция и границы текущей реализации приведены в `TECHNICAL_GUIDE_RU.md` и `reports/turbulence/V3_INTEGRATION_RU.md`.
